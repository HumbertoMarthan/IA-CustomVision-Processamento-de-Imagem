...
public class CustomVisionSamples {
	...
	public static void runSample(TrainingApi trainer, PredictionEndpoint predictor) {
		try {
			// This demonstrates how to create an image classification project, upload
			// images,
	...
	public static void ImageClassification_Sample(TrainingApi trainClient, PredictionEndpoint predictor) {
		try {
			...
			/*
			 * // create hemlock tag Tag hemlockTag = trainer.createTag()
			 * .withProjectId(project.id()) .withName("Hemlock") .execute();
			 * 
			 * // create cherry tag Tag cherryTag = trainer.createTag()
			 * .withProjectId(project.id()) .withName("Japanese Cherry") .execute();
			 * 
			 * // create preto tag Tag pretoTag = trainer.createTag()
			 * .withProjectId(project.id()) .withName("Preto") .execute();
			 * 
			 * // create verde tag Tag verdeTag = trainer.createTag()
			 * .withProjectId(project.id()) .withName("Verde") .execute();
			 */
			// create azul tag
			Tag azulTag = trainer
					.createTag()
			...
			/* EXEMPLOS
			 * System.out.println("Adding images..."); for (int i = 1; i <= 10; i++) {
			 * String fileName = "hemlock_" + i + ".jpg"; byte[] contents =
			 * GetImage("/Hemlock", fileName); AddImageToProject(trainer, project, fileName,
			 * contents, hemlockTag.id(), null); }
			 */
			/*
			 * for (int i = 1; i <= 10; i++) { String fileName = "japanese_cherry_" + i +
			 * ".jpg"; byte[] contents = GetImage("/Japanese Cherry", fileName);
			 * AddImageToProject(trainer, project, fileName, contents, cherryTag.id(),
			 * null); }
			 */
		...
		try {
			...
			for (Prediction prediction : results.predictions()) {
				System.out.println(String.format("\t%s: %.2f%% at: %.2f, %.2f, %.2f, %.2f", prediction.tagName(),
						prediction.probability() * 100.0f, prediction.boundingBox().left(),
						prediction.boundingBox().top(), prediction.boundingBox().width(),
